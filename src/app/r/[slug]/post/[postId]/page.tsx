    ...
    <div>
      <div className="h-full flex flex-col sm:flex-row items-center sm:items-start justify-between ">
        <Suspense fallback={<PostVoteShell />}>
          ...
          <PostVoteServer postId={cachedPostData?.id ?? post.id}
            getData={async () => {
        ...
        <div className="sm:w-0 w-full flex-1 bg-white p-4 rounded-sm">
          <p className="max-h-40 mt-1 truncate text-xs text-gray-500 ">
            Posted by u/
            {post?.author.username ?? cachedPostData?.authorUsername}{" "}
            <span className="font-bold">
              {formatTimeToNow(
                new Date(
                  post?.createdAt ??
                    (cachedPostData?.createdAt
                      ? cachedPostData?.createdAt
                      : new Date())
                )
              )}
          ...
          <EditorOutput
            content={
              post?.content ??
              (cachedPostData?.content ? cachedPostData?.content : null)
            }
          />
